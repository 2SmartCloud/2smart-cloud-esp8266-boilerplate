...
bool Homie::Init(String user_hash, String host, uint16_t broker_port, String token, MQTT_CALLBACK_SIGNATURE) {
...
bool Homie::Publish(const Property& property, String attribute, String value, bool retained) {
    ...
    return this->mqtt_client_->Publish((topic + attribute).c_str(), value.c_str(), retained);
}
...
bool Homie::Publish(const Node& node, String attribute, String value, bool retained) {
    ...
    return this->mqtt_client_->Publish(topic.c_str(), value.c_str(), retained);
}
...
bool Homie::Publish(const Device& device, String attribute, String value, bool retained) {
    ...
    return mqtt_client_->Publish(topic.c_str(), value.c_str(), retained);
}

bool Homie::SubscribeTopic(const Property& property) {
    String topic = this->GetTopicForEntity(property);
    return this->mqtt_client_->Subscribe((topic + "/set").c_str());
}
